// Vendored from optionslab (optionstrat/strategies/detectType.ts), keep in sync with the source.
// Local changes: local assert instead of node "assert"; optional chaining and guards
// required by noUncheckedIndexedAccess.

function assert(value: unknown): asserts value {
  if (!value) {
    throw new Error("detectType: unexpected empty position");
  }
}

import { type Strategy, strategies } from "./strategies";
 ...
 */
// eslint-disable-next-line max-lines-per-function, complexity -- FIXME: refactor, 233 lines (max 150); complexity 268 (max 25)
export function detectType(positions: (OptionContract | StockPosition)[], isCashSecured = false): Strategy {
  ...
  const sameSize = optionPositions.every((p) => Math.abs(p.position) === Math.abs(optionPositions[0]?.position ?? 0));
        ...
        if (sameSize) {
          ...
          if (optionPositions[0].type === "put" && optionPositions[1].type === "call") {
            if (optionPositions[0].strike < optionPositions[1].strike) {
              // eslint-disable-next-line max-depth -- FIXME: refactor, nesting depth 5 (max 4)
  ...
  // Four positions
    ...
    if (optionPositions.length === 4 && sameSize && sameExpiration) {
      ...
      if (
        ...
        optionPositions[1].type === "call" &&
        optionPositions[2].type === "put" &&
