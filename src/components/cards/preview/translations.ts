// Ukrainian translations of IBKR statement texts for the printable preview

export function translatePeriod(period: string | undefined) {
  if (!period) {
    return "";
  }
...
export function translateAccount(str: string | undefined) {
  if (!str) {
    return "";
  }
...
export function translateCode(code: string | undefined) {
  if (!code) {
    return "";
  }
  const codes = [
