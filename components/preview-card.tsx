  ...
  return (
    <>
      ...
      <table className="table-auto w-full">
        ...
        <tbody>
          {rows.map((tr, index) => (
            <TradesTableRow key={index} tr={tr} />
          ))}
        </tbody>
      </table>
    </>
  ...
  return (
    <div className="space-y-4">
      ...
      <table className="table-auto w-full">
        ...
        <tbody>
          {rows.map((tr, index) => (
            <tr key={index} className={cn("border-t first:border-t-0", index === 0 && "bg-secondary", index === rows.length - 1 && "bg-secondary font-semibold")}>
              {Array.from(tr.querySelectorAll("td")).map((td, tdIndex) => (
                <Cell key={tdIndex} className={cn(tdIndex === 2 && "text-right")} colSpan={td.colSpan}>
                  {td.textContent?.replace("USD", "Долар США")?.replace("Total", "Загалом")}
                </Cell>
              ))}
            </tr>
          ))}
...
function translateCode(code: string | undefined) {
  ...
  const codes = [
    ...
    {
      code: "Pr",
      description: "Part or all of this transaction was executed by the Exchange as a Crossing by IB against an IB affiliate and is therefore classified as a Principal and not an agency trade",
      translation: "Угода (частково або повністю) виконана біржею як кросинг IB проти афілійованої сторони (principal угода)",
    },
    ...
    {
      code: "RPA",
      description: "IB acted as agent for both the fractional share portion and the whole share portion of this trade; the fractional share portion was executed by an IB Affiliate as riskless principal.",
      translation: "IB діяв як агент для дробової та повної частини угоди; дробова частина виконана афілійованою стороною IB як безризикова угода",
    },
